...
export const StepsForm = (): ReactElement => {
      ...
      <ul className="-mt-4">
        {!fields.length && (
          <div className="h-12 w-full flex justify-center items-center  mt-4">
            <p className="bg-neutral-focus p-4 rounded-lg">Pas d&apos;étape</p>
          </div>
        )}
        {fields.map((field, index) => (
          <li key={`step-input-${index}`}>
            <TextInput
              type="hidden"
              {...register(`steps.create.${index}.index` as const)}
            />
            <div className="flex gap-2 items-end">
              <TextInput
                label={`Description de l'étape ${index + 1}`}
                placeholder="Quelles sont les actions à faire ?"
                {...register(`steps.create.${index}.body` as const)}
              />
